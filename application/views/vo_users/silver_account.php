                ...
                <a href="<?=base_url($init['langu'].'/vo/users/list')?>" class="btn btn-primary">Back</a>
      			...
      			<div class="widget stacked">
					...
					<div class="widget-header">
						<i class="icon-star"></i>
						<h3>User <?=$user_details['name']?> Silver account (<?=$user_details['silver_account_no']?>) log</h3>
					</div> <!-- /widget-header -->
					...
					<div class="widget-content">
                    	...
                    	?>
                    		<tr><td><?=$v['created_date']?></td><td><?=$v['action'].' '.$v['remarks']?></td><td><?=$v['oz']>0?$v['oz']:''?></td><td><?=$v['oz']<0?$v['oz']:''?></td><td><?=$balance?></td></tr>
                      ...
                      <!-- Get DO Modal content-->
                            ...
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="oz" id="oz" placeholder="Enter the ounce, e.g. 100.20">
                                </div>
...
$('#silver-form').on('submit', function(e) { //use on if jQuery 1.7+
                e.preventDefault();  //prevent form from submitting             

                var token = "<?=$init['token']?>";
                
                var formData = $("#silver-form :input").serializeArray();              
                var obj = {};
                ...
                $.ajax({
                  type: "POST",
                  url: "<?=base_url($init['langu'].'/api/insertUserSilver')?>",
                  data: JSON.stringify(obj),
                  success: function(json){
                        if(json.status == "OK") {
                            alert(json.result.success_text);
                            location.href="<?=base_url($init['langu'].'/vo/users/silver_account/'.$user_id)?>"                                                      
                        } else {
                            alert(json.result);
