				...
				<div class="container">
					<div class="row">
						...
						<div class="main col-md-8">
							...
							<!-- page-title start -->
							<!-- ================ -->
							<h1 class="page-title"><?=$init['lang']['USD']?> <?=$init['lang']['Account History']?> (<span id="usd_account_no"></span>)</h1>
...
$(document).ready(function(){
	...
	$.getJSON("<?=base_url($init['langu'].'/api/getUSDHistory')?>/"+token, function(result){
        if(result.status == "OK") {
        	...
        	$("#usd_account_no").html(result.result.usd_account_no);

        	if(result.result.history.length > 0) {
        		for(var i=0; i < result.result.history.length; i++) {

        			balance += result.result.history[i].amount;
        			...
        			content += "<td><div align='right'>"+(result.result.history[i].amount>0?result.result.history[i].amount:'')+"</div></td>";
        			content += "<td><div align='right'>"+(result.result.history[i].amount<0?result.result.history[i].amount:'')+"</div></td>";
