				...
				<div class="container">
					<div class="row">
						...
						<div class="main col-md-8">
							...
							<!-- page-title start -->
							<!-- ================ -->
							<h1 class="page-title"><?=$init['lang']['TRANSFER USD TO SILVER ACCOUNT']?></h1>
							<div class="separator-2"></div>
							<!-- Forms start -->
							<!-- ============================================================================== -->							
							<br>
							<p><?=$init['lang']['usd2silver_desc']?></p>							
							
							<form id="usd-transfer-form" role="form">
								<div class="form-group">
									<label for="usd_account_no"><?=$init['lang']['From']?> <?=$init['lang']['USD account balance: (USD)']?>: <span class="label label-danger" id="usd_account_balance"></span></label>
									<input type="text" class="form-control" name="usd_account_no" id="usd_account_no" placeholder="USD account no." readonly="readonly">
								</div>

								<div class="form-group">
									<label for="silver_account_no"><?=$init['lang']['To']?> <?=$init['lang']['Silver account balance: (oz)']?>: <span class="label label-success" id="silver_account_balance"></span></label>
								...
								<div class="col-xs-6">
									<div class="form-group">
										...
										<input type="text" class="form-control" name="amount" id="amount" placeholder="Please enter the amount you wish to transfer" value="100" onblur="calculateSilverRate(this.value);">
								...
								<div class="col-xs-6">
									<div class="form-group">
										...
										<input type="text" class="form-control" name="oz" id="oz" placeholder="Please enter the ouce you wish to buy" onblur="calculateUSDRate(this.value);">
...
function calculateTotal(){
	...
	var total_fee = (amount+transaction_fee);

	$("#total_fee").val(total_fee);

	if(usd_balance >= total_fee) {
...
function calculateSilverRate(amount){
	 ...
	 $.ajax({
					  type: "GET",
					  url: "/"+langu+"/api/getUSD2SilverRate/"+amount,
					  data: "",
...
function calculateUSDRate(oz){
	 ...
	 $.ajax({
					  type: "GET",
					  url: "/"+langu+"/api/getUSD2SilverRateOz/"+oz,
					  data: "",
					  success: function(json){
					  		if(json.status == "OK") {
					  			$("#rate").html(json.result.rate);
...
$(function () {    
	...
	calculateSilverRate($("#amount").val());
...
$('#usd-transfer-form').on('submit', function(e) { //use on if jQuery 1.7+
				...
				$.ajax({
				  type: "POST",
				  url: "/"+langu+"/api/transferUSDtoSilver",
				  data: JSON.stringify(obj),
				  success: function(json){
				  		if(json.status == "OK") {
				  			alert(json.result.success_text);
				  			location.href="<?=base_url($init['langu'].'/usd_history')?>"				  							  			
				  		} else {
				  			alert(json.result);
